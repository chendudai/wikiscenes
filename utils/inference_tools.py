...
class ResultWriter:
    ...
    def save(self, img_path, img_orig, all_masks, labels, pads, gt_mask):
        ...
        # CRF
        pred_crf = crf_inference(img_orig255, merged_mask, t=10, scale_factor=1, labels=merged_mask.shape[0])
        ...
        if self.verbose:
            ...
            crf_path = os.path.join(self.root, "vis", img_name + '_pred_crf.png')
            # gt_path = os.path.join(self.root, "vis", img_name + '._gt.png')
            pred_o = np.round(pred_o * 255.).astype(np.uint8)
            pred_crf_o = np.round(pred_crf_o * 255.).astype(np.uint8)
            # mask_gt_o = np.round(mask_gt_o * 255.).astype(np.uint8)
...
def get_inference_io(method_name):
    ...
    elif method_name == "singlescale":
        return MergeSingleScale, MultiscaleLoader
    else:
