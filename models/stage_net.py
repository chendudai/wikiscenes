...
from models.backbones.mobilenet import mobilenet_v2
...
def network_factory(cfg):
    ...
    if cfg.BACKBONE == "resnet38":
        ...
        backbone = ResNet101
    elif cfg.BACKBONE in ["resnet18", "squeezenet", "mobilenet_v2"]:
        print("Use torchvision provided backbone")
        backbone = None
    else:
    ...
    if cfg.MODEL != 'cls':
        ...
        class SoftMaxAE(backbone):
            ...
            def forward(self, y, y_raw=None, labels=None, layer="decoder"):
                ...
                out_feature["conv3"] = self._backbone['conv3']
                ...
                out_feature["multi"] = torch.cat([out_feature["conv3"], out_feature["decoder"]], 1)

                return cls, cls_fg, {"cam": masks, "dec": masks_dec, "full": masks_full, "feature": out_feature[layer]}, self._mask_logits, pseudo_gt, loss_mask
        ...
        class Classification(nn.Module):
            ...
            def __init__(self, config, pre_weights=None, num_classes=21, dropout=True):
                ...
                if self.cfg.BACKBONE == "resnet18":
                    ...
                    self.model_ft.fc = nn.Linear(num_ftrs, num_classes-1)
                    self.model_ft.layer4.register_forward_hook(hook)
                elif self.cfg.BACKBONE == "resnet50":
                    self.model_ft = models.resnet50(pretrained=True)
                    num_ftrs = self.model_ft.fc.in_features
                    self.model_ft.fc = nn.Linear(num_ftrs, num_classes-1)
                    self.model_ft.layer4.register_forward_hook(hook)
                elif self.cfg.BACKBONE == "squeezenet":
                    self.model_ft = models.squeezenet1_0(pretrained=True)
                    self.model_ft.classifier[1] = nn.Conv2d(512, num_classes-1, kernel_size=(1,1), stride=(1,1))
                    self.model_ft.num_classes = num_classes - 1
                    self.model_ft.features[5].squeeze_activation.register_forward_hook(hook)
                elif self.cfg.BACKBONE == "mobilenet_v2":
                    self.model_ft = mobilenet_v2(pretrained=True)
                    self.model_ft.classifier[1] = nn.Linear(1280, num_classes-1)
                    self.model_ft.features[4].conv[0].register_forward_hook(hook)

                if torch.cuda.is_available():
                    self.model_ft = self.model_ft.cuda()

            def parameter_groups(self, _, __):
                return self.parameters()

            def forward(self, y, _, labels=None, __=None):
                cls = self.model_ft(y)
                return cls, torch.zeros_like(cls), {"feature": self.out_feature}, torch.zeros_like(cls), None, None
